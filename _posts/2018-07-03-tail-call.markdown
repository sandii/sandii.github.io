...
function fibonacci (n, rs1 = 1, rs2 = 1) {
	if ( n <= 1 ) return rs2;
	return fibonacci.bind(null, n - 1, rs2, rs1 + rs2);
}
// 调用
trampoline( factorial(100) );
trampoline( fibonacci(100) );
```

2. 尾递归优化的真实实现比蹦床函数更复杂一些
```
function tco (f) {
	let value;
	let active = false;
...
let fibonacci = tco(function (n, rs1 = 1, rs2 = 1) {
	if ( n <= 1 ) return rs2;
